...
def Study(gaussian_N=13, gaussian_std=1.2, lamb=1e-2):
    ...
    img_solved = solve(img_tests, gau_rgbs, lamb, 8)
    # save result image
    img_out = np.round(255*np.clip(img_solved, 0.0, 1.0)).astype('uint8')
    imageio.imwrite(f'./result/zebra_test_{gaussian_N}_{gaussian_std}_{lamb}.png', img_out)
    # check answer
    img_urs_path = f'./result/zebra_test_{gaussian_N}_{gaussian_std}_{lamb}.png'
...
if __name__ == '__main__':
    ...
    #    Study(gaussian_N=i, gaussian_std=1.2, lamb=1e-2)
    for i in  np.arange(0.3, 1.2, 0.3):
        Study(gaussian_N=13, gaussian_std=i, lamb=1e-2)
    #for i in [1e-2, 5e-2, 1e-1, 0.5, 1]:
    #    Study(gaussian_N=13, gaussian_std=1.2, lamb=i)